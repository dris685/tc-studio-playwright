...
test.describe.skip('Review Audits Suite', () => {
  ...
  test('Verify Album Section', async ({ }) => {
    ...
    // Verify Track Level on Content Review Page (TC-STUDIO)
    const trackCountTitleOnTCStudio = await tabStudio.locator("div[data-testid='songs'] .tcstudio-MuiCardHeader-content span.tcstudio-MuiCardHeader-title")
    expect(await trackCountTitleOnTCStudio.textContent()).toEqual('Tracks (3)')
